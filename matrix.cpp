...
// Параллельное перемножение целочисленных матриц (OpenMP)
vector<vector<int>> multiplyIntMatricesOMP(const vector<vector<int>>& A, const vector<vector<int>>& B) {
    int m = A.size();
    int n = A[0].size();
    int p = B[0].size();
...
void writeFullResults(const string& filename, const vector<vector<int>>& result, double time_ms, int size) {
    ofstream file(filename);
    if (!file.is_open()) {
        throw runtime_error("Cannot open file: " + filename);
    }
...
int main() {
    ...
    try {
        for (int size = min_size; size <= max_size; size += step) {
            ...
            auto C = multiplyIntMatricesOMP(A, B);
